...
postsController.create = async (req, res, next) => {
    ...
    try {
        ...
        let postSaved = (await post.save()).populate('images');
...
postsController.edit = async (req, res, next) => {
    try {
        ...
        const { described, images, videos } = req.body;
        let dataImages = images && Array.isArray(images) ? [] : postFind.images;
        if (Array.isArray(images)) {
            for (const image of images) {
                // check is old file
                if (image) {
        ...
        // let dataVideos = videos && Array.isArray(videos) ? [] : postFind.videos;
...
postsController.list = async (req, res, next) => {
    try {
        ...
        if (req.query.userId) {
            ...
            const strangerPosts = await PostModel.find({
                author: {
                    $nin: listIdFriends,
                },
            })
            ...
            posts = [...friendPosts, ...strangerPosts].slice(0, 20);
        }
        let postWithIsLike = [];
