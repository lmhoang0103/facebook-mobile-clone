...
friendsController.setAccept = async (req, res, next) => {
  try {
    ...
    if (!friend) {
      res.status(404).json({
        code: 404,
        message: 'Không tồn tại lời mời từ user này',
        success: false,
      });
      return;
    }
...
friendsController.blockList = async (req, res, next) => {
  try {
    const currentUser = await UserModel.findById(req.userId);
    const listBlockDiary = currentUser.blocked || [];
    ...
    res.status(200).json({
      code: 200,
      message: 'Danh sách người bị chặn',
      data: {
